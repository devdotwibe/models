...
if (!empty($_SESSION['log_user_id'])) {
    $log_user_id = (int) $_SESSION['log_user_id'];
    $get_modal_user = DB::query("SELECT * FROM model_user WHERE id = %i", $log_user_id);

    if ($get_modal_user) {
        $user       = $get_modal_user[0];
        $modalname  = ucfirst(!empty($user['username']) ? $user['username'] : $user['name']);
        $as_a_model = $user['as_a_model'];
