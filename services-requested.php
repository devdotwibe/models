...
where model_unique_id='".$userDetails['id']."' ";
$sort_by = ' ORDER BY tb.id desc ';
$booking_group_show = DB::query($stringQuery." ".$sort_by);
...
<?php echo '<b>'.$row_it['username'].'</b> has requested International Tour at '.$row_it['meeting_date'].''; ?>
<div class="mt-4">
<b>ID</b>: #<?=$row_it['id']?> | <b>Coins</b>: <i class="far fa-coins"></i> <?=$row_it['amount']?>
</div>
</td>
<td style="width:90px">
...
echo '<div style="padding:10px;text-align:center">You dont have any request for International Tour</div>'; 
}
?>
</div>
</div><!--International Tours -->


<div id="gs-tab" class="tab-pane fade ">
<div>
<?php
if($booking_group_show){
foreach($booking_group_show as $row_it){

?>
<table>
<tr>
<td style="vertical-align: middle;"><?php echo '<b>'.$row_it['username'].'</b> has at '.$row_it['meeting_date'].''; ?>
<div class="mt-4">
<b>ID</b>: #<?=$row_it['id']?> | <b>Coins</b>: <i class="far fa-coins"></i> <?=$row_it['coins']?>
</div>
</td>
<td style="width:90px">
<button class="btn btn-primary" data-toggle="modal" data-target="#gs-model-<?php echo $row_it['id']; ?>">View Details</button></td>
<td style="width:130px">
<?php
if($row_it['status']=='accept'){
?>
...
?>
<a class="btn btn-xs btn-success" href="service-processing.php?service-name=group_show&service_id=<?php echo $row_it['id']; ?>&action=accept">Accept</a>
&nbsp;
<a class="btn btn-xs btn-danger" href="service-processing.php?service-name=group_show&service_id=<?php echo $row_it['id']; ?>&action=reject">Reject</a>
<?php
}
...
<div id="gs-model-<?php echo $row_it['id']; ?>" class="modal fade" role="dialog">
<div class="modal-dialog">
...
<div class="row">
  <div class="col-md-6 mb-3 d-flex justify-content-between ">
	...
	<div class="col-md-6 mb-3 d-flex justify-content-between ">
    	<div><strong>Time:</strong></div>   	
        <div><?php echo $row_it['meeting_time'] ?></div>
	</div>

	<div class="col-md-6 mb-3 d-flex justify-content-between ">
    	<div><strong>Members:</strong></div>   	
        <div><?php echo $row_it['total_member']; ?></div>
	</div>
...
<tr>
<td style="vertical-align: middle;">
<?php echo '<b>'.$name2.'</b> has requested group show at '.$row_da['meeting_date'].''; ?>
<div class="mt-4">
<b>ID</b>: #<?=$row_da['id']?> | <b>Amount</b>: $<?=$row_da['amount']?>
</div></td>
...
<tr>
<td style="vertical-align: middle;">
<?php echo '<b>'.$result_ma['username'].'</b> has requested group show at '.$result_ma['meeting_date'].''; ?>
<div class="mt-2">
<b>ID:</b> #<?=$result_ma['id']?>  | <b>Coins</b>: <i class="far fa-coins"></i> <?=$result_ma['amount']?>
</div>
</td>
