...
if(isset($_SESSION["VALID_USER_ID"]))
{
	...
	$check_user_details = mysqli_query("select * from `follow_and_unfollow_users_table` where `email` = '".mysqli_real_escape_string($page_owner)."'");
	
	//Get all the logged in user information from the database users table
	$get_user_details = mysqlii_fetch_array($check_user_details);
...
{
	$check_following_or_not = mysqli_query("select * from `follow_and_unfollow_activity` where `page_owners_emails` = '".mysqli_real_escape_string($page_owner)."' and `followers_emails` = '".mysqli_real_escape_string($_SESSION["VALID_USER_ID"])."'");
...
$check_users_in_the_system = mysqli_query("select * from `follow_and_unfollow_users_table` order by `id` desc limit 100");
