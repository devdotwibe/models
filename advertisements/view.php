...
<!-- Main Content -->
    <main class="main">
        <div class="container">
			...
			$similar_adv_query = "select tb.*,mu.age from banners tb join model_user mu on mu.id=tb.user_id where tb.category='".$form_data['category']."' LIMIT 3";
			
			if(DB::numRows($similar_adv_query) < 3){
				$similar_adv = DB::query("select tb.*,mu.age from banners tb join model_user mu on mu.id=tb.user_id LIMIT 3");
			}else{
