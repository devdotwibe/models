	...
	<script>
		...
		jQuery('.paymentsub_btn').bind('click', async function(e) {
			...
			if (cardElementEmpty == true) {
				jQuery(".cardnumber_req").html('Required.');
				stats = false;
			...
			if (cardElementInvalid == true) {
				jQuery('.cardnumber_error').html('Your card number is invalid.');
				stats = false;
				jQuery(".spinner-border").hide();
			} else jQuery('.cardnumber_error').html('');
			...
			if (cardElementexpInvalid == true) {
				jQuery('.cardnumber_error').html('Your card expairy is invalid.');
				stats = false;
				jQuery(".spinner-border").hide();
			} else jQuery('.cardnumber_error').html('');
			...
			if (cardElementcvvEmpty == true) {
				jQuery(".cardnumber_req").html('CVC Required.');
				stats = false;
				jQuery(".spinner-border").hide();
			} else jQuery(".cardnumber_req").html('');
				...
				try {
					...
					if (response.status === 'success' && response.message !== '') {
						const {
							error,
							paymentIntent
						} = await stripe.confirmCardPayment(response.message, {
							payment_method: {
